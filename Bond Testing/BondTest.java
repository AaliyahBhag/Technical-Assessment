...
class BondTest {
    ...
    void setUp() {
        ...
        bond = new Bond(settlementDate, maturityDate, lastCouponDate, couponRate, yieldToMaturity, faceValue, dayCountConvention, couponFrequency);
    }
    ...
    void testYieldLessThanCouponRate() {
        ...
        double faceValue = 100.00;
        double dayCountConvention = 365;
        int couponFrequency = 2;

        Bond bondWithLowerYield = new Bond(settlementDate, maturityDate, lastCouponDate, couponRate, yieldToMaturity, faceValue, dayCountConvention, couponFrequency);

        double cleanPrice = bondWithLowerYield.calculateCleanPrice();
        double allInPrice = bondWithLowerYield.calculateAllInPrice();

        assertTrue(cleanPrice > faceValue, "Clean price should be greater than face value when yield is less than coupon rate.");
        assertTrue(allInPrice > faceValue, "Dirty price should be greater than face value when yield is less than coupon rate.");
    }
    ...
    void testYieldGreaterThanCouponRate() {
        ...
        Bond bondWithHigherYield = new Bond(settlementDate, maturityDate, lastCouponDate, couponRate, yieldToMaturity, faceValue, dayCountConvention, couponFrequency);

        double cleanPrice = bondWithHigherYield.calculateCleanPrice();
        double allInPrice= bondWithHigherYield.calculateAllInPrice();

        assertTrue(cleanPrice < faceValue, "Clean price should be less than face value when yield is greater than coupon rate.");
        assertTrue(allInPrice < faceValue, "Dirty price should be less than face value when yield is greater than coupon rate.");
    }
}
